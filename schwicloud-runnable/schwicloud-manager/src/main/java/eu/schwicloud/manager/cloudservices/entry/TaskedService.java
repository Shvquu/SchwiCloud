package eu.schwicloud.manager.cloudservices.entry;
...
import eu.schwicloud.manager.cloudservices.interfaces.ITaskedService;
...
import eu.schwicloud.process.ServiceProcess;
...
public class TaskedService implements ITaskedService {
    ...
    public void handelLaunch() {
        if (this.getEntry().getNode().equals("InternalNode")){
            Driver.getInstance().getTerminalDriver().log(Type.INFO, Driver.getInstance().getLanguageDriver().getLang().getMessage("task-service-start")
                    .replace("%service%", entry.getServiceName())
                    .replace("%node%", entry.getNode())
    ...
    public void handelScreen() {
        if (this.getEntry().getNode().equals("InternalNode")){
            if (Driver.getInstance().getMessageStorage().openServiceScreen){
                ...
                Driver.getInstance().getMessageStorage().screenForm = getEntry().getServiceName();
    ...
    public void handelQuit() {
        ...
        Driver.getInstance().getWebServer().updateRoute("/cloudservice/general", new ConfigDriver().convert(list));
        if (this.getEntry().getNode().equals("InternalNode")){
            if (Driver.getInstance().getMessageStorage().openServiceScreen && Driver.getInstance().getMessageStorage().screenForm.equals(getEntry().getServiceName()) ){
                Driver.getInstance().getTerminalDriver().leaveSetup();
                process.handelConsole();
                base.cancel();
            }
            Driver.getInstance().getTerminalDriver().log(Type.INFO, Driver.getInstance().getLanguageDriver().getLang().getMessage( "task-service-shutdown")
                    .replace("%service%", entry.getServiceName()).replace("%uuid%", entry.getUUID()));
